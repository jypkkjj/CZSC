"""案例 14b：QuantAxis 真实数据跑 Event 策略回测 + wbt HTML 报告

与 14_tushare_event_backtest.py 完全同构，只是数据源换成
`czsc.connectors.qa_connector`（对内网 QA webserver，QA 实测 sub-table stock）。
...
运行：
    # 默认连接 http://192.168.50.11:8010，可用 QA_DOMAIN 改成其它实例
    uv run --no-sync python docs/examples/14_qa_event_backtest.py

产物：
    _output/14_qa_event_backtest/
        ├── single_event.html
        └── multi_event.html

⚠️ 与 14_tushare_event_backtest.py 唯一的语义差异（来自 QA server 响应口径）：
    - QA 日线 vol 字段单位是「手」、amount 单位是「元」（Tushare 是「手 / 千元」）。
      本文件仅用 30 分钟数据，回测逻辑与手续费/绩效口径不变。
    - QA 30/60 分钟 K 线需要 sdt/edt 形成"跨日"区间才有数据；qa_connector
      已经在内部把 end 扩展到次日 00:00，对调用方透明。
"""
...
from czsc.connectors.qa_connector import get_raw_bars

# 所有案例统一把产物落到 _output/
OUTPUT_DIR = Path(__file__).resolve().parent / "_output" / "14_qa_event_backtest"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 全局回测参数
SYMBOL = "588200"   # QA webserver 只识别 6 位裸 code；qa_connector 已自动剥后缀
BASE_FREQ = "30分钟"
SDT_DATA = "20200101"     # QA 当前可见大致 2025-09 起 30min 数据，起点放早点
EDT_DATA = "20260612"
...
def run_one(tag: str, strategy: CzscStrategyBase, bars: list, sdt: str) -> dict[str, float]:
    ...
    generate_backtest_report(
        ...
        title=f"案例 14b - {tag} 回测报告（QuantAxis 真实数据）",
        fee_rate=FEE_RATE,
        weight_type="ts",
        yearly_days=yearly,
    )
...
def main() -> None:
    # 1) 用 QuantAxis webserver 拉真实 30 分钟 K 线（QA 仅支持原始单位，不复权）
    print(f"[数据] 正在从 QuantAxis webserver 获取 {SYMBOL} {BASE_FREQ} K 线...")
    bars = get_raw_bars(
        symbol=SYMBOL,
        freq=BASE_FREQ,
        sdt=SDT_DATA,
        edt=EDT_DATA,
        fq="不复权",   # QA webserver 当前不支持复权，固定为「不复权」
    )
    print(f"[数据] {bars[0].symbol} {bars[0].freq.value} 共 {len(bars)} 根；"
          f"{bars[0].dt} ~ {bars[-1].dt}")

    if len(bars) < 200:
        raise RuntimeError(f"QA 返回数据过少 ({len(bars)} bars)，请检查服务器与小符号区间")
