...
| **15 分钟**   | `(无) 服务端没表`                          | 客户端 `czsc.resample_bars` 从 5min 重采样 | Tushare（SQL 内部用） | `trade_time`   |
| **30 分钟**   | `(无) 服务端没表`                          | 客户端 `czsc.resample_bars` 从 5min 重采样 | 同上 | `trade_time`   |
| **60 分钟**   | `(无) 服务端没表`                          | 客户端 `czsc.resample_bars` 从 5min 重采样 | 同上 | `trade_time`   |
...
### 6.4 分钟级客户端重采样 F15 / F30 / F60

服务端只有 min1 / min5，**不存在 min15 / min30 / min60**：

```python
FREQ_DDB_TBL = {
    Freq.F15: Same as Freq.F5   # → 客户端 resample_bars 从 5min 升采样到 15min
    Freq.F30: Same as Freq.F5   # → 客户端 resample_bars 从 5min 升采样到 30min
    Freq.F60: Same as Freq.F5   # → 客户端 resample_bars 从 5min 升采样到 60min
}
```

[`get_raw_bars`](../../czsc/connectors/ddb_connector.py) 内对 F15/F30/F60 显式走 5min base 重采样。数学一致性：F30 一天 8 根 ≈ F15 一天 16 根 / 2，F15：F30 =**2:1**。
...
| 6.4  | 服务端只有 min1/min5，F15/F30/F60 走客户端重采样          | `get_raw_bars` 中的 `if freq in (F15, F30, F60)` 分支     |
